...
public class PartitioningHashGroupBy{
    ...
    public PartitioningHashGroupBy(int in_group, int in_agg, int out_group, int out_agg, Aggregation agg, int parallel_lvl)
    {
    ...
    public Record[] apply(String file_path) {
        ...
        try {
            ...
            this.input = Record.fromArray(myArray);
            myReader.close();
        ...
        Record[] output = new Record[full_size];
        ...
        public void run() {
            for(Record t: input) {
                ...
                if(group % n_partitions == partition_no) { //modulus partitioning.
                    Record group_val = partitions[partition_no].get(group);
                    if(group_val == null) {
